...
def min_move(board, token, depth, alpha, beta):
    opponent = "xo"["ox".index(token)]
    ...
    if not list_of_moves:
        return max_move(board, opponent, depth-1, alpha, beta)
    for move in list_of_moves:
        if alpha < beta:
            new_board = make_move(board, token, move)
            results.append(max_move(new_board, opponent, depth-1, alpha, beta))
            beta = min(results)
    return min(results)

    

def max_move(board, token, depth, alpha, beta):
    opponent = "xo"["ox".index(token)]
    list_of_moves = possible_moves(board, token)
    results = [alpha,]
    if depth == 0 or game_over(board):
        return score(board)
    if not list_of_moves:
        return min_move(board, opponent, depth-1, alpha, beta)
    for move in list_of_moves:
        if alpha < beta:
            new_board = make_move(board, token, move)
            results.append(min_move(new_board, opponent, depth-1, alpha, beta))
            alpha = max(results)
    return max(results)
...
def find_next_move(board, token, depth):
    ...
    for move in possible_moves(board, token):
        new_board = make_move(board, token, move)
        if opponent == "x":
            results[max_move(new_board, opponent, depth-1, alpha, beta)] = move
        else:
            results[min_move(new_board, opponent, depth-1, alpha, beta)] = move
    if token == "o":
